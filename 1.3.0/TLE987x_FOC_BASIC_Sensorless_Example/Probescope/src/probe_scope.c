...
struct  scope_ch
{
    ...
    CPU_INT08U  DataType;                                                     // Source data type:
                                                                              //    0 = CPU_INT08U
                                                                              //    1 = CPU_INT08S
                                                                              //    2 = CPU_INT16U  (only if
                                                                              // 'PROBE_SCOPE_16_BIT_EN' is set to 1)
                                                                              //    3 = CPU_INT16S  (only if
                                                                              // 'PROBE_SCOPE_16_BIT_EN' is set to 1)
                                                                              //    4 = CPU_INT32U  (only if
                                                                              // 'PROBE_SCOPE_32_BIT_EN' is set to 1)
                                                                              //    5 = CPU_INT32S  (only if
                                                                              // 'PROBE_SCOPE_32_BIT_EN' is set to 1)
                                                                              //    6 = CPU_FP32    (only if
                                                                              // 'PROBE_SCOPE_32_BIT_EN' is set to 1)
    CPU_BOOLEAN           BitEn;                                              // Bit mode enabled (1) or disabled (0)
    CPU_INT08U            BitSel;                                             // Selected bit in bit mode (0..63)
    CPU_INT08U          * DataAddr;                                           // Address of source data (i.e. address of
                                                                              // symbol assigned to channel)
 ...
 *********************************************************************************************************
 */
static PROBE_SCOPE_CH ProbeScope_Ch1;                                         // Up to 8 scope channels (minimum 1
                                                                              // channel)
...
static CPU_INT08U     ProbeScope_Mode;                                        // Scope Mode:
                                                                              ...
                                                                              //     1 = Single
                                                                              //     2 = Continuous
                                                                              //     3 = Triggered (always assumes
                                                                              // pre-triggering)
static CPU_INT08U ProbeScope_TrigChSel;                                       // Channel that will be monitored for the
                                                                              // trigger:
                                                                              //     1 = Channel #1
                                                                              //     2 = Channel #2
                                                                              //     3 = Channel #3
                                                                              ...
                                                                              //     7 = Channel #7
                                                                              //     8 = Channel #8
volatile CPU_INT32U  ProbeScope_TrigDispPos;                                  // Position of the trigger on the screen
                                                                              // (in sample number):
                                                                              //     0                           = Far
                                                                              // left,  identical to 100%
                                                                              // post-triggering
                                                                              //     PROBE_SCOPE_MAX_SAMPLES - 1 = Far
                                                                              // right, identical to 100% pre-triggering
volatile CPU_BOOLEAN ProbeScope_TrigFlag;                                     // Flag indicating that we satisfied the
                                                                              // trigger
volatile CPU_INT32U  ProbeScope_TrigHoldOff;                                  // Determines how many samples we will
                                                                              // hold off before retriggering
static CPU_INT32U    ProbeScope_TrigHoldOffCtr;
volatile CPU_INT32U  ProbeScope_TrigIx;                                       // Position in the sample buffer where the
                                                                              // trigger was found
...
static CPU_INT32U    ProbeScope_SamplesPreTrigCtr;                            // Minimum number of samples needed before
                                                                              // we are allowed to trigger
static CPU_INT32U    ProbeScope_SamplesPostTrigCtr;                           // Determines how many samples will be
                                                                              // take after a trigger has been detected:
                                                                              //     In 100% post-trigger,
...
void ProbeScope_Init(uint32_t sampling_clk_hz)
{
    ...
    ProbeScope_TrigSlope          = PROBE_SCOPE_TRIG_POS;                    // We assume a positive going signal by
                                                                             // default
...
static void ProbeScope_ModeContinuous(void)
{
    if (ProbeScope_DataRdyFlag == 0)                                            // Read one buffer's worth of data
    {
        ...
        else
        {
            ...
            ProbeScope_SampleChAll();                                           // Get the first sample to create a
                                                                                // 'previous' value for triggering
    ...
    else
    {
        ...
        switch (ProbeScope_State)
        {
            case PROBE_SCOPE_STATE_START:
                ...
                ProbeScope_TrigHoldOffCtr     = 0;
                ProbeScope_SamplesPostTrigCtr = 0;
                ProbeScope_SamplesPreTrigCtr  = ProbeScope_TrigDispPos;             // Minimum number of samples
                                                                                    // pre-trigger
                ProbeScope_SampleChAll();                                           // Get the first sample to create a
                                                                                    // 'previous' value for triggering
            ...
            case PROBE_SCOPE_STATE_SAMPLING_PRE_TRIG:
                ...
                if (ProbeScope_SamplesPreTrigCtr > 0)                               // We need a minimum number of
                ...
                if (ProbeScope_TrigFlag == 1)                                       // Remaining number of samples to
                                                                                    // collect before data available
                {
            ...
            case PROBE_SCOPE_STATE_DATA_RDY:
                ...
                if (ProbeScope_TrigHoldOffCtr > 0)                                  // Hold-off before beeing able to
                                                                                    // re-trigger
                {
                ...
                {
                    if (ProbeScope_DataRdyFlag == 0)                                // Give Probe time to collect and
                                                                                    // display the waveform(s)
                    {
                        ProbeScope_TrigFlag          = 0;
                        ProbeScope_SamplesPreTrigCtr = ProbeScope_TrigDispPos;      // Minimum number of samples
                                                                                    // pre-trigger
...
static void ProbeScope_IsTrig(void)
{
    ...
    if (p_ch->En == 1)
    {
        p_sample_prev = &p_ch->Samples[ProbeScope_SampleIxPrev];          // Point to previous position in sampling
                                                                          // buffer
        p_sample_cur  = &p_ch->Samples[ProbeScope_SampleIxCur];           // Point to current  position in sampling
                                                                          // buffer
